...
function EmpHistory({user, notify}) {
  ...
  const grouped = records.reduce((a,r)=>{(a[r.date]=a[r.date]||[]).push(r);return a;},{});
  if(loading) return <Spinner/>;
  return(
    <div style={{padding:20}}>
      ...
      {Object.entries(grouped).sort((a,b)=>b[0].localeCompare(a[0])).map(([ds,recs])=>{
        const cin=recs.find(r=>r.check_in_time), cout=recs.find(r=>r.check_out_time);
        const worked=cin?.worked_mins;
        return(
          <div key={ds} style={{background:C.white,borderRadius:18,padding:16,marginBottom:10,boxShadow:`0 2px 8px ${C.g300}33`,borderLeft:`4px solid ${cin?.is_late?C.amber:C.g500}`}}>
            ...
            <div style={{display:"flex",gap:16}}>
              <span style={{color:C.g600,fontSize:14,fontWeight:600}}>▶ {cin?.check_in_time?.slice(0,5)||"—"}</span>
              <span style={{color:C.gr500,fontSize:14}}>⏹ {cin?.check_out_time?.slice(0,5)||"—"}</span>
...
function EmpSalary({user, notify}) {
  ...
  const now = new Date();
  useEffect(()=>{
    GET("/api/salary-report",{year:now.getFullYear(),month:now.getMonth()+1})
      .then(r=>{ const me=r.report?.find(e=>e.id===user.id); setReport(me||null); })
      .catch(e=>notify(e.message,"error"))
      .finally(()=>setLoading(false));
  },[]);
  if(loading) return <Spinner/>;
  ...
  return(
    <div style={{padding:20}}>
      ...
      <div style={{background:`linear-gradient(135deg,${C.g800},${C.g600})`,borderRadius:24,padding:24,marginBottom:20}}>
        ...
        <p style={{color:"rgba(255,255,255,0.55)",fontSize:13}}>of {fmt(user.salary||0)}/month</p>
