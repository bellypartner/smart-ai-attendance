// ============================================================
// SmartAi Attendance — Full-Stack Server
// Express serves React PWA (dist/) + API on ONE Railway service
// No Netlify needed
// ============================================================
...
// Resolve org_id: super_admin can pass ?org_id, others use their own
const orgId = (req) =>
...
app.post('/api/orgs', auth(['super_admin']), async (req, res) => {
  ...
  try {
    ...
    const { rows } = await client.query(
      ...
      [req.body.name, req.body.code.toUpperCase()]
    );
...
app.patch('/api/branches/:id', auth(['super_admin','org_admin','branch_admin']), async (req, res) => {
  ...
  await db('UPDATE branches SET name=$1,address=$2,lat=$3,lng=$4,radius=$5,is_active=$6,updated_at=now() WHERE id=$7',
...
app.get('/api/schedules', auth(), async (req, res) => {
  ...
  `;
  const params = [oid, from || today, to || today];
...
app.get('/api/attendance', auth(), async (req, res) => {
  ...
  if (date) { params.push(date); sql += ` AND ar.date = $${params.length}`; }
    ...
    params.push(from || today); params.push(to || today);
...
app.post('/api/attendance/checkin', auth(['employee']), async (req, res) => {
  try {
    ...
    const existing = await db('SELECT id FROM attendance_records WHERE employee_id=$1 AND date=$2', [req.user.id, date]);
    ...
    const shiftMins = toMins(shift.start_time);
    const nowMins = toMins(time);
    let lateMins = nowMins - shiftMins;
    if (lateMins < -720) lateMins += 1440; // overnight shift correction
...
app.post('/api/attendance/checkout', auth(['employee']), async (req, res) => {
  const { date, time } = nowIST();
  const { rows } = await db('SELECT * FROM attendance_records WHERE employee_id=$1 AND date=$2', [req.user.id, date]);
...
app.post('/api/attendance/admin-mark', auth(['super_admin','org_admin','branch_admin']), async (req, res) => {
  ...
  await db(`
    ...
    DO UPDATE SET check_in_time=$5,check_out_time=$6,worked_mins=$7,is_late=$8,late_mins=$9,
                  approval_status='approved',admin_edited=true,edited_by=$10,edited_at=now(),notes=$11,updated_at=now()
...
app.patch('/api/approvals/:id', auth(['super_admin','org_admin','branch_admin']), async (req, res) => {
  ...
  await db('UPDATE attendance_records SET approval_status=$1 WHERE id=$2', [req.body.status, rows[0].record_id]);
  res.json({ ok: true });
});
...
app.get('/api/leaves', auth(), async (req, res) => {
  const oid = orgId(req);
  const { from, to, employee_id } = req.query;
  let sql = 'SELECT l.*, u.name AS employee_name FROM leaves l JOIN users u ON u.id=l.employee_id WHERE l.org_id=$1';
  const params = [oid];
...
app.get('/api/salary-report', auth(['super_admin','org_admin','branch_admin']), async (req, res) => {
  ...
  const [{ rows: emps }, { rows: settRows }] = await Promise.all([
    db(`SELECT u.id,u.name,u.designation,u.salary,b.name AS branch_name
        FROM users u LEFT JOIN branches b ON b.id=u.branch_id
        WHERE u.org_id=$1 AND u.role='employee' AND u.is_active=true ORDER BY u.name`, [oid]),
  ...
  const report = await Promise.all(emps.map(async (emp) => {
    ...
    return { ...emp, presentDays, lateDays, unauthLeaves, noShows, casualUsed,
  ...
  res.json({ year: y, month: m, from, to, report,
             total: report.reduce((s, r) => s + r.netEarned, 0) });
});
